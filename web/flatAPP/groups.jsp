...
<body>
	...
	<div id="page-content" >
		...
		<div class="block block-tiles block-tiles-animated clearfix" id="Primary">
		<%List<Group> primaryGroups = dao.GroupModule.getGroups(user.getId(),"Primary"); %>
		<% for (Group group : primaryGroups) {%>
			<a href="group.jsp?gid=<%=group.getGroup_id()%>" class="tile tile-themed themed-background-ocean tile-width-2x">
			<i class="fa fa-users"></i>
			<div class="tile-info"><%=group.getTitle()%></div>
			</a>
		<%}%>
		</div>
		<div class="block block-tiles block-tiles-animated clearfix" id="General">
			<%List<Group> alliedGroups = dao.GroupModule.getGroups(user.getId(),"General"); %>
